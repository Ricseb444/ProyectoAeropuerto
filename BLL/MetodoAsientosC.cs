...
namespace BLL
{
    public class MetodoAsientosC
    {
        public readonly StringBuilder stringBuilder = new StringBuilder();
        private readonly PasajeroDAL PDAL;

        public MetodoAsientosC()
        {
        ...
        {
            List<Pasajero> ListaPasajeros = PDAL.GetAll("pasajeros3");
        ...
        public int AgregarPasagero(int numA, Pasajero pasajero)
        {
            ...
            if (asientosLibres > 0)
            {
                ...
                {
                    if (PDAL.GetByid(pasajero.Cedula,"pasajeros3") == null)
                    {
                ...
                stringBuilder.Clear();
                PDAL.Insert(pasajero, "pasajeros3");
                ...
                if (item is Pasajero pasajero)
                {
                    if (string.Equals(pasajero.Clase, clase, StringComparison.OrdinalIgnoreCase))
                    {
                        if (int.Equals(pasajero.Asiento, asiento))
                        {
                            ...
                            {
                                PDAL.Delete(pasajero.Cedula, "pasajeros3");
